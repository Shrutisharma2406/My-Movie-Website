...
fetch(json_url).then(Response => Response.json()).then((data)=>{
    ...
    data.forEach(element => {
        ...
        <img src="${sposter}" alt="">
                        <div class="cont">
                            <h3>${name}</h3>
                            <p>${genre} ,${date} ,<span>IMBD</span><i class="bi bi-star-fill">${imdb}</i></p>
                        </div>
        `
    ...
    search_input.addEventListener('keyup', () => { 
        ...
        for (let index = 0; index < a.length; index++) { let b= a[index].getElementsByClassName ('cont' )[0]; 
...
movies.addEventListener('click',()=>{
    ...
    let movies_a = data.filter(ele =>{
        return  ele.type == "movie";
    });
    movies_a.forEach((ele,i) => {
        let{ name, imdb, date, sposter, bposter, genre, url  }=ele;
        let card = document.createElement('a');
        card.classList.add('card');
        card.href = url;
